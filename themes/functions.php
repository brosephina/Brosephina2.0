...
{
	$br = CBrosephina::getInstance();
	if($br->user['isAuthenticated'])
	{
		if($br->user['hasRoleAdmin']) 
		{
			return true;
		}
	}
...
function userLinks($theLink, $info)
{
  global $br;
  
  $links = $br->request->CreateUrl($theLink);
  $linky = "<a href='{$links}'>{$info}</a>";
  
  return $linky;
}

function links2($theLink, $info)
{
  global $br;
